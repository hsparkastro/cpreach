...
try:
    from univariate import dynamics
except ImportError:
    from examples.univariate.univariate import dynamics
...
cwd = os.getcwd()
if "examples/univariate" not in cwd:
    os.chdir("./examples/univariate")
...
def test(config_path):
    ...
    x = valset['x'][:, :-1]
    y = valset['x'][:, 1:]
    u = np.empty((x.shape[0], y.shape[1], 0))
